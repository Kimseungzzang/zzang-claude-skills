...
**CTX와 OPEN 항목은 간결하게 유지하세요.**
이 필드들은 무기한으로 누적됩니다. 장황한 항목은 CURRENT.ctx를 부풀려 로드 시 토큰을 낭비합니다. 사실당 짧은 한 문장으로 충분합니다.

**TRIED 항목은 실패 이유를 구체적으로 적으세요.**
`TRIED`는 Claude가 이미 실패한 접근법을 다시 제안하는 것을 방지합니다. 실패 이유가 구체적일수록 유용합니다: `Redis pub/sub(동시 쓰기 시 레이스 컨디션)`이 `Redis pub/sub(실패)`보다 훨씬 낫습니다.
